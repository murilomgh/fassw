...
public class GroundingDados {
    ...
    static Integer contador = 1; //um contador que estabelece um nome unico para cade elemento que nao possua um nome
    Document documento;

    public GroundingDados(String entrada, String saida) {
     ...
     */
    private void percorrerSchemas(Node types) {
            ...
            if (namespace != null && namespace.equals("http://www.w3.org/2001/XMLSchema")) {              
                ...
                while (elemento != null) {
                    ...
                    if (elemento.getNodeType() == Node.ELEMENT_NODE) {
                        ...
                        switch (nomeElemento) {
                            case "element" :
                                saida.append(mapearElement(elemento));
                                break;
                            case "simpleType" :
                                saida.append(transformarSimpleType(elemento));
                                break;
                            case "complexType" :
                                saida.append(transformarComplexType(elemento));
                                break;
                            case "attribute" :
                            ...
                            case "attributeGroup" :
                                ...
                                break;
                            case "group" :
                                saida.append(transformarGroup(elemento));
                                break;
                            default :
                                break;
                        }
                ...
                try {
                    String caminho = this.caminho.concat("ontologia" + contador + ".wsml");
                    pw = new PrintWriter(caminho, "UTF-8");
                    pw.write(saida.toString());
                    saida.delete(0, saida.length());
                } 
                catch (FileNotFoundException | UnsupportedEncodingException ex) {
                    Logger.getLogger(GroundingDados.class.getName()).log(Level.SEVERE, null, ex);
                } 
                finally {
                    pw.close();
                }
    ...
    private String transformarComplexType(Node raiz) {
        StringBuilder saida = new StringBuilder();
        String nomeDoElemento;
        Node name = raiz.getAttributes().getNamedItem("name");
        Node id = raiz.getAttributes().getNamedItem("id");
        ...
        if (name == null) {
            if (id == null) { 
                ...
                raiz.getAttributes().setNamedItem(name);
            }
            else {
        ...
        saida.append("concept ").append(nomeDoElemento).append("\n");
        
        //percorrer os atributos e gerar annotations
        saida.append("\t").append("annotations").append("\n");
        saida.append("\t\t").append("xmlType hasValue ").append("\"complexType\"").append("\n");
        Node atributo;
        
        atributo = raiz.getAttributes().getNamedItem("abstract");
        if (atributo != null) 
            saida.append("\t\t").append("abstract hasValue \"").append(atributo.getNodeValue()).append("\"\n");
        
        atributo = raiz.getAttributes().getNamedItem("mixed");
        if (atributo != null)
            saida.append("\t\t").append("mixed hasValue \"").append(atributo.getNodeValue()).append("\"\n");
            
        //TODO implementar transformacoes das restricoes/extensoes dentro de 'block' e 'final'
        atributo = raiz.getAttributes().getNamedItem("block");
        if (atributo != null)
            saida.append("\t\t").append("block hasValue \"").append(atributo.getNodeValue()).append("\"\n");
            
        atributo = raiz.getAttributes().getNamedItem("final");
        if (atributo != null) 
            saida.append("\t\t").append("final hasValue \"").append(atributo.getNodeValue()).append("\"\n");
            ...
            if (filho.getNodeType() == Node.ELEMENT_NODE) {
                ...
                switch (componente) {
                    case "simpleContent":
                        ...
                        saida.append(transformarElementoBase(base));
                        break;
                    case "complexContent":
                        saida.append(transformarComplexContent(filho));
                        break;
    ...
    private String transformarUnion(Node elemento, String indentacao) {
        ...
        saida.append("\n").append("concept ").append(id).append(" subConceptOf ").append("{ ");
        ...
        while (filho != null) {
            if (filho.getNodeType() == Node.ELEMENT_NODE && filho.getLocalName().equals("annotation")) {
                saida.append(transformarAnnotation(filho, indentacao + "\t"));
                break;
            }
            filho = filho.getNextSibling();
        }
    ...
    private Object transformarList(Node elemento, String indentacao) {
        ...
        if (itemType != null) {
            saida.append(itemType.getNodeValue()).append("\n");
        } else {
            Node filho = elemento.getFirstChild();
            ...
            while (filho != null) {
                if (filho.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    saida.append(nomeSimpleType).append("\n\n");
                    saida.append(transformarSimpleType(filho));
                }
                filho = filho.getNextSibling();
            } 
        }
    ...
    String transformarAttributeGroup(Node elemento) {
        throw new UnsupportedOperationException("Ainda nao implementado."); //To change body of generated methods, choose Tools | Templates.
    }
    ...
    private String processarAnnotations(Node elemento) {
        StringBuilder saida = new StringBuilder();
        saida.append("\t").append("annotations").append("\n");
